...
public class Scan {
  ...
  /*  Constructor -- here initializes variables...
  ...
  public Token scan() { 
    if( got_eof ) {
      ...
      if(putback) 
	    ...
	    if ( myisalpha((char) c) ) /* identifier. */
		    return new Token(TK.ID, buildID(), linenumber);
      else if ( myisdigit((char) c) ) /* number. */
          ...
          case EOF:
		      ...
		      case '#': // gobble comments
			      ...
			      break;
		      default:
			      System.err.println( "scan: line "+linenumber+ " bad char (ASCII " + c + ")");
			      break;
		    }
	    }
	  }
  ...
  private Token ccase2(char c1, char c2, TK r) {
	  ...
	  if (c == c2) {
	    ...
	    System.err.println("scan: got got " + c1 + " missing " + c2 + " (got ASCII " + c + ")");
